...
namespace recs
{
	...
	private:
		...
		inline bool run_system_for_entity(const recs::cursor in_cursor, const recs::count in_count, const recs::index in_index)
		{
			...
			{
				// The modified type is an enforced reject, so the entity never has it
				// on entry: always set.
				[:System:](get_system_parameter_value<ParameterTypes>(in_cursor, in_count, in_index)...);
